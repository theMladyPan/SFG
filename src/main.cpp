...
float convertRawAccel(int aRaw, double gain = 1.0) {
    double g = (aRaw * 2.0) / 32768.0;
    double acc = g * 9.81;
    return acc * gain;
}
...
void loop() {
  ...
  double measuredGravity = vectorLength(x_null, y_null, z_null);
  ESP_LOGI("main", "Gravity: %f", measuredGravity);
  gain = 9.81 / measuredGravity;
  ESP_LOGI("main", "Calibrated gain: %f", gain);
  ...
  while(1) {
    ...
    */
    ...
    for(int i=0; i<samples; i++)
    {
      ...
      BMI160.readAccelerometer(axRaw, ayRaw, azRaw);
      ax = convertRawAccel(axRaw, gain);
      ay = convertRawAccel(ayRaw, gain);
      az = convertRawAccel(azRaw, gain);
